...
with db.connect() as session:
    ...
    playlists = pd.read_sql(select(db.Label.name, db.Label.playlist_id).order_by(db.Label.name), session.connection())

stats = df.groupby("label")["popularity"].describe().sort_values("50%", ascending=False).reset_index()
