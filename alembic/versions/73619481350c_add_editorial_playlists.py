...
def upgrade() -> None:
    ...
    op.add_column(
        "playlist",
        sa.Column("image_url", sa.String(), nullable=True, comment="Cover image URL of the playlist"),
    )
    label_table = sa.table("label", sa.column("playlist_id"), sa.column("playlist_image_url"))
    ...
    op.create_index(op.f("ix_snapshot_track_id"), "snapshot", ["track_id"], unique=False)
    ...
    with op.batch_alter_table("track_popularity") as batch_op:
        batch_op.alter_column("Spotify popularity", new_column_name="popularity", comment="Spotify popularity")
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("track_popularity") as batch_op:
        batch_op.alter_column("popularity", new_column_name="Spotify popularity", comment=None)
