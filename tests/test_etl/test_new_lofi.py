...
@iterator_to_list
def playlists(
    session: db.Session, playlist_generator: PlaylistGenerator
) -> Iterator[db.Playlist]:
    for _ in range(2):
...
@load_data_output
def lofi_label(
    session: db.Session, playlists: list[db.Playlist], label_generator: LabelGenerator
) -> db.Label:
    return label_generator.generate(
        is_lofi=True, playlist_id=playlists[0].id, playlist=playlists[0]
    )
...
def non_lofi_label(
    ...
    return label_generator.generate(
        is_lofi=False, playlist_id=playlists[1].id, playlist=playlists[1]
    )
...
@load_data_output
def new_album(
    session: db.Session, lofi_label: db.Label, album_generator: AlbumGenerator
) -> db.Album:
    return album_generator.generate(
        label=lofi_label, label_name=lofi_label.name, release_date=datetime.date.today()
    )
...
@load_data_output
def non_lofi_album(
    session: db.Session, non_lofi_label: db.Label, album_generator: AlbumGenerator
) -> db.Album:
    return album_generator.generate(
...
@pytest.fixture
def albums(
    new_album: db.Album, non_lofi_album: db.Album, old_album: db.Album
) -> list[db.Album]:
...
@iterator_to_list
def tracks(
    session: db.Session, albums: list[db.Album], track_generator: TrackGenerator
) -> Iterator[db.Track]:
...
@pytest.fixture
def new_lofi_tracklist(
    session: db.Session, track_popularities: list[db.TrackPopularity]
) -> list[db.Track]:
    ids = get_new_lofi_tracklist(session)
    tracks = list(
        session.execute(select(db.Track).where(db.Track.id.in_(ids))).scalars()
    )
    assert len(tracks) == len(ids)
    return tracks


def test_get_new_lofi_tracklist_is_not_empty(
...
def test_get_new_lofi_tracklist_only_returns_lofi_labels(
    lofi_label: db.Label, new_lofi_tracklist: list[db.Track]
) -> None:
    for track in new_lofi_tracklist:
        assert track.album.label_name == lofi_label.name


def test_get_new_lofi_tracklist_returns_recent_tracks(
    new_album: db.Album, new_lofi_tracklist: list[db.Track]
) -> None:
    for track in new_lofi_tracklist:
...
def test_get_new_lofi_tracklist_returns_all_recent_tracks(
    session: db.Session, new_album: db.Album, new_lofi_tracklist: list[db.Track]
) -> None:
...
def test_get_new_lofi_tracklist_is_ordered_by_decreasing_popularity(
    new_lofi_tracklist: list[db.Track],
) -> None:
